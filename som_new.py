...
def main():
    ...
    max_num = data.max().max()
    square = data.divide(max_num)
    square_data = square.to_numpy()

    # create som object with "Rows:30, Cols:30,Dim:2"
    som_square = SOM(20,20)
    som_square.fit(square_data)

    som_square.plot_density_map(square_data)
    som_square.plot_distance_map()
    targets = range(40)
    som_square.plot_point_map(square_data, range(len(square_data)), range(len(square_data)), "asdf")
